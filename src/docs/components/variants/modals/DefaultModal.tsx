...
const DefaultModal = (): JSX.Element => {
  ...
  const actionsNoClose = <>
    <Button variant="assertive">Confirm</Button>
    <Button onClick={() => modalRef3.current?.close()}>Cancel</Button>
  </>;
  ...
  return (
    <Example isFlex>
      ...
      <Modal logo={BSCLogo} logoAlt="Boston scientific logo" title="Modal title" actions={actions} ref={modalRef2}>
        This is an example of a modal.
      </Modal>
      <Button type="button" aria-haspopup="true" onClick={() => modalRef3.current?.showModal()}>Open modal</Button>
      <Modal hasClose={false} title="Modal title" actions={actionsNoClose} ref={modalRef3}>
        This is an example of a modal.
      </Modal>
    </Example>
  );
