...
import {
  GetCodeStandardQuery,
  useGetCodeStandardQuery,
...
const CodeStandards = (): JSX.Element => {
  ...
  const [navItems, setNavItems] = useState<NavItemSecondary[]>(
    [] as NavItemSecondary[]
  );
  const [codeStandardData, setCodeStandardData] = useState<
    GetCodeStandardQuery["codeStandard"]
  ...
  useTitle({
    titlePrefix: `${codeStandardData?.name} - Code Standards`,
  });
  ...
  return (
    <Layout>
      <PageTemplate
        ...
        seoMetaDescription={
          codeStandardData?.pageProperties?.seoMetaDescription || ""
        }
