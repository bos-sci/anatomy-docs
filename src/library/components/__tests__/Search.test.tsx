...
describe('Search', () => {
  ...
  it('Should render clear button only when there is a value', () => {
    const { rerender } = render(<Router><Search label="Search" /></Router>);

    expect(screen.getAllByRole('button')).toHaveLength(1);
    rerender(<Router><Search label="Search" value="test" /></Router>);
    expect(screen.getAllByRole('button')).toHaveLength(2);
  });

  it('Should clear input value on press of clear button', async () => {
    const user = userEvent.setup();
    render(<Router><Search label="Search" value="test" /></Router>);
  ...
  it('Should run onChange function on change of input value', async () => {
    ...
    render(<Router><Search label="Search" onChange={onChange} /></Router>);
  ...
  it('Should open autocomplete panel when typing unless hasAutoComplete is false', async () => {
    const user = userEvent.setup();
    const { rerender } = render(<Router><Search label="Search" searchResults={searchResults} /></Router>);
    screen.getByRole('combobox').focus();
    await user.keyboard('bread');
    expect(screen.getByRole('list')).toBeInTheDocument();

    await rerender(<Router><Search label="Search" searchResults={searchResults} hasAutocomplete={false} /></Router>);
  ...
  it("Should change aria-activedescendant value to id of first result on press of down arrow", async () => {
    const user = userEvent.setup();
    render(<Router><Search label="Search" searchResults={searchResults} /></Router>);
