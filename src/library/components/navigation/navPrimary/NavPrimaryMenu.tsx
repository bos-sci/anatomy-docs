...
const NavPrimaryMenu = forwardRef((props: Props, ref: ForwardedRef<HTMLDivElement>): JSX.Element => {
  ...
  return (
    <div ref={ref} id={props.menuId} className={"lib-nav-menu" + (props.isMenuOpen ? ' open' : '') + (props.isIntermediateNav ? ' intermediate' : '')}>
      {props.history.length > 0 &&
        <Button
          type="button"
          variant="subtle"
          className="lib-nav-menu-back"
          onClick={() => props.popHistory()}>
          Back
        </Button>
      }
      <div className="lib-nav-menu-panels">
        <NavPrimaryList
      ...
      {(props.history.length === 0 && props.utilityItems) &&
        <ul className="lib-nav lib-nav-block">
          {props.utilityItems.map((item, i) => (
            <li key={'utilityItem' + i} className="lib-nav-item">
              {item.children ?
                <Dropdown key={'utilityItem' + item.text + i} triggerText={item.text} className="lib-nav-link">
                  {item.children.map((child, childI) => (
                    <Link key={child.text + childI} href={child.href} to={child.slug}>{child.text}</Link>
                  ))}
                </Dropdown>
                : <Link to={item.slug} href={item.href} className="lib-nav-link">{item.text}</Link>
              }
            </li>
          ))}
        </ul>
